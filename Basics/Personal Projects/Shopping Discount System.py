...
def hitung_diskon(total, is_member, promo):
    ...
    if is_member and total > 500000:
        ...
        print(f"Diskon dasar (20%): Rp{round(memberdisc):,}")
    elif is_member:
        memberdisc = total * 10 / 100
        print(f"Diskon dasar (10%): Rp{round(memberdisc):,}")
    else:
        print("Diskon dasar: Rp0.00")
    
    if promo == "HEMA" and (total - memberdisc) >= 100000:
        promodisc = (total - memberdisc) * 5 / 100
        print(f"Diskon tambahan (5% dari {round(total - memberdisc):,}): Rp{round(promodisc):,}")
    else:
    ...
    print(f"Total akhir: Rp{round(total - memberdisc - promodisc):,}")
